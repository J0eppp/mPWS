	...
	def run(self):
		...
		for period in periods:
			...
			for component in components:
				...
				if len(sys.argv) >= 6:
					url = 'https://api2020.luchtmeetnet.nl/measurements?component_id={component_id}&start={start}&end={end}&station_id={station_id}&limit=999&zero_fill=1&order_direction=asc&status={status}'.format(component_id = component_id, start = start, end = end, station_id = self.id, status = sys.argv[7])
				try:
...
def get_valid_values(input_folder, output_folder):
